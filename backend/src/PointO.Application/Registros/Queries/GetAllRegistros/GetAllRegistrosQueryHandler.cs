...
public sealed class GetAllRegistrosQueryHandler(IRegistroRepository repository)
    : IRequestHandler<GetAllRegistrosQuery, IEnumerable<RegistroDto>>
{
    ...
    {
        var registros = await repository.ObterTodosAsync(request.DataInicio, request.DataFim, request.Empresa, ct);
